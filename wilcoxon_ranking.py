...
def pairs_metrics_multi(method_names, dataset_names, metrics, experiment_names, methods_alias=None, streams_alias=None, metrics_alias=None, treshold=0.05, title=False):
    if metrics_alias is None:
        metrics_alias = metrics
    if methods_alias is None:
        methods_alias = method_names
    if streams_alias is None:
        streams_alias = dataset_names[0].split("/")[0]
    ...
    for method_name in method_names:
        for stream_name in dataset_names:
    ...
    for index_i, experiment_name in enumerate(experiment_names):
        for index_j, (metric, metric_a) in enumerate(zip(metrics, metrics_alias)):
            ...
            # Pair tests
            # --------------------------------------
            for stream in tqdm(dataset_names, "Rank %s %s" % (experiment_name, metric)):
            ...
            try:
                ...
                pass
            axes[index_i, index_j].set_xlim([0, len(dataset_names)])
...
def pairs_metrics_multi_ensemble(method_names, dataset_names, metrics, experiment_names, data, methods_alias=None, streams_alias=None, metrics_alias=None, treshold=0.05, title=False):
    if metrics_alias is None:
        metrics_alias = metrics
    if methods_alias is None:
        methods_alias = method_names
