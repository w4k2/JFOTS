...
class _JFOTSProblem(ElementwiseProblem):
    def __init__(
        ...
        *,
        ...
        super().__init__(n_var=self.n_variables, n_obj=2, xl=0, xu=1, type_var=bool)
...
class JFOTSSolution:
    def __init__(
        ...
        self.feature_mask = individual[:-4]
        self.type_mask = individual[-4:]
...
class JFOTS:
    ...
    def fit_resample(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        self.objectives = -result.F[order]
        self.history = [-r.opt.get("F") for r in result.history]
        self.solutions = []

        for individual, objectives in zip(result.X[order], self.objectives):
